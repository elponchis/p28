 ...
 * both — and, more to the point, may have only one. Web Push is skipped silently when VAPID is
 * unconfigured.
 *
...
async function sendChatMessagePushes(
  ...
  // No Expo tokens is the normal state of a browser-only deployment, not a reason to stop: web
  // push is a separate transport to the same people, and returning here skipped it entirely.
  const sendResult =
    messages.length > 0
