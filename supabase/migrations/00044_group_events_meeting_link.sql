-- Online meeting link (Zoom, Meet, etc.), separate from physical location.

ALTER TABLE public.group_events
  ADD COLUMN IF NOT EXISTS meeting_link TEXT NOT NULL DEFAULT '';

DROP FUNCTION IF EXISTS public.create_group_event_with_discussion(uuid, text, text, timestamptz, boolean, text);
DROP FUNCTION IF EXISTS public.update_group_event(uuid, text, text, timestamptz, boolean, text);
...
GRANT EXECUTE ON FUNCTION public.create_group_event_with_discussion(UUID, TEXT, TEXT, TIMESTAMPTZ, BOOLEAN, TEXT, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.update_group_event(UUID, TEXT, TEXT, TIMESTAMPTZ, BOOLEAN, TEXT, TEXT) TO authenticated;
