...
check(
  realtimeContent.includes('subscribe') && realtimeContent.includes('unsubscribe'),
  'contract realtime defines subscribe/unsubscribe'
);

// errors.ts: ApiError shape
const errorsContent = fs.readFileSync(path.join(contractsDir, 'errors.ts'), 'utf8');
check(
  errorsContent.includes('message') &&
    (errorsContent.includes('code') || errorsContent.includes('code?')),
  'contract errors defines ApiError shape'
);

// dto.ts: camelCase DTOs (User, Session)
const dtoContent = fs.readFileSync(path.join(contractsDir, 'dto.ts'), 'utf8');
check(
  dtoContent.includes('User') && dtoContent.includes('Session'),
  'contract dto defines User and Session'
);
...
check(
  facadeContent.includes('auth') &&
    facadeContent.includes('data') &&
    facadeContent.includes('realtime'),
  'facade exports auth, data, realtime'
);

// Adapter directory exists
check(fs.existsSync(adaptersSupabase), 'lib/api/adapters/supabase/ exists');
check(
  fs.existsSync(path.join(adaptersSupabase, 'index.ts')),
  'lib/api/adapters/supabase/index.ts exists'
);
...
check(
  ...
  '.env.example has SUPABASE_URL and SUPABASE_ANON_KEY'
);
...
check(
  facadeContent.includes('export type') && facadeContent.includes('ApiError'),
  'facade re-exports contract types including ApiError'
);
